-- Fix moderation protection system by removing all auto-moderation triggers and installing proper protection

-- 1. Remove ALL auto-moderation triggers that automatically set content to pending
DROP TRIGGER IF EXISTS trigger_auto_moderate_reported_content ON public.reports;
DROP TRIGGER IF EXISTS auto_moderate_on_report_creation ON public.reports;

-- 2. Now safely remove the auto_moderate_reported_content function
