-- Fix the SQL syntax error in analyze_content_for_spam function
...
BEGIN
  ...
  -- Check for duplicate content (FIXED: removed function name prefix)
  SELECT COUNT(*) INTO similar_count
  FROM public.content_analysis ca
  WHERE ca.content_hash = content_hash
  ...
  INSERT INTO public.content_analysis (
    ...
    content_hash, content_type, spam_indicators, is_spam, LEAST(confidence, 1.00)
  );
