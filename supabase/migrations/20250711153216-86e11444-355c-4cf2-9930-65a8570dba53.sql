-- Drop and recreate get_enriched_posts function to include parent post moderation status
...
CREATE OR REPLACE FUNCTION public.get_enriched_posts(p_topic_id uuid, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, content text, author_id uuid, topic_id uuid, parent_post_id uuid, created_at timestamp with time zone, updated_at timestamp with time zone, moderation_status text, author_username text, author_avatar_url text, parent_post_content text, parent_post_author_username text, parent_post_author_avatar_url text, parent_post_created_at timestamp with time zone, parent_post_moderation_status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
