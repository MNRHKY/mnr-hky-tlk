...
-- Fix record_enhanced_anonymous_activity function
...
BEGIN
  -- Insert or update tracking record with properly qualified column references
  ...
  ON CONFLICT (ip_address, anonymous_post_tracking.session_id, anonymous_post_tracking.fingerprint_hash) 
  DO UPDATE SET 
    post_count = anonymous_post_tracking.post_count + CASE WHEN record_enhanced_anonymous_activity.content_type = 'post' THEN 1 ELSE 0 END,
    topic_count = anonymous_post_tracking.topic_count + CASE WHEN record_enhanced_anonymous_activity.content_type = 'topic' THEN 1 ELSE 0 END,
    last_post_at = now();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Fix check_enhanced_anonymous_rate_limit function
...
BEGIN
  ...
  -- Get or create tracking record with properly qualified parameters
...
-- Update the record_enhanced_anonymous_activity function call to use new parameter names
CREATE OR REPLACE FUNCTION record_enhanced_anonymous_activity(
  user_ip inet, 
  p_session_id text, 
  p_fingerprint_hash text DEFAULT NULL,
