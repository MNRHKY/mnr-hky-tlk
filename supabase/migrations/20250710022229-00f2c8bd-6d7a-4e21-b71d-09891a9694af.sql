-- Phase 1: Disable all moderation immediately

-- 1. Update all categories to not require moderation
UPDATE public.categories 
SET requires_moderation = false 
WHERE requires_moderation = true;

-- 2. Approve all pending topics
UPDATE public.topics 
SET moderation_status = 'approved' 
WHERE moderation_status = 'pending';

-- 3. Approve all pending posts
UPDATE public.posts 
SET moderation_status = 'approved' 
WHERE moderation_status = 'pending';

-- 4. Update get_hot_topics function to filter by approved status
CREATE OR REPLACE FUNCTION public.get_hot_topics(limit_count integer DEFAULT 25)
 RETURNS TABLE(id uuid, title text, content text, author_id uuid, category_id uuid, is_pinned boolean, is_locked boolean, view_count integer, reply_count integer, vote_score integer, last_reply_at timestamp with time zone, created_at timestamp with time zone, updated_at timestamp with time zone, username text, avatar_url text, category_name text, category_color text, category_slug text, slug text, hot_score numeric, last_post_id uuid)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
